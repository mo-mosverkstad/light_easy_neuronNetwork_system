...
# Generate toy data: two classes of points
np.random.seed(42)
N = 200  # Number of samples
X_class0 = np.random.randn(N//2, 2) + np.array([0.0, -2.0])
X_class1 = np.random.randn(N//2, 2) + np.array([2.5, 2.0])
X = np.vstack([X_class0, X_class1])           # Shape (N, 2)
y = np.array([0]*(N//2) + [1]*(N//2))         # Labels 0/1
y = y.reshape(-1, 1)

# Standardization (optional)
X = (X - X.mean(axis=0)) / X.std(axis=0)

# Model structure: 2 -> 16 -> 1 (Sigmoid)
input_dim = 2
hidden_dim = 16
output_dim = 1
lr = 0.1
...
print("Training complete. Final accuracy:", accuracy(forward(X)[0], y))